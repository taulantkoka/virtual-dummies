// vd_afs.cpp
// Virtual Dummy Adaptive Forward Stepwise.
#include "vd_afs.hpp"
using DummyLaw = VDDummyLaw;

// ========================================================================
// Local helpers (same streaming GEMV + Cholesky append as VD-OMP)
// ========================================================================
namespace {
...
// Constructors
// ========================================================================
VD_AFS::VD_AFS(const double* Xptr, int n, int p,
               const double* yptr, int ny,
               int num_dummies,
               const VDOptions& o)
  : n_(n), p_(p), L_(num_dummies),
    rho_(std::max(0.0, std::min(o.rho, 1.0))),   // clamp to (0,1]
    opt_(o), rng_(opt_.seed),
    X_(Xptr, n, p),
    y_(yptr, ny)
{
  if (rho_ <= 0.0) rho_ = 1e-6;  // avoid degenerate zero step
  init_state_();
}

VD_AFS::VD_AFS(const Eigen::Ref<const MatC>& X_in,
               const Eigen::Ref<const Vec>&  y_in,
               int num_dummies,
               const VDOptions& o)
  : VD_AFS(X_in.data(), (int)X_in.rows(), (int)X_in.cols(),
...
Eigen::VectorXi VD_AFS::active_indices() const {
...
// ========================================================================
// Basis helpers (identical to VD-LARS / VD-OMP)
// ========================================================================
std::optional<Vec> VD_AFS::orthonormalize_(const Vec& v) const {
...
Vec VD_AFS::project_to_Vperp_(const Vec& z) const {
  const int nb = basis_size_();
  if (nb == 0) return z;
...
// ========================================================================
// Virtual-dummy machinery (identical to VD-OMP)
// ========================================================================
void VD_AFS::ensure_vd_rows_capacity_(int need) {
...
void VD_AFS::initialize_virtual_dummies_() {
...
void VD_AFS::update_virtual_dummies_() {
...
void VD_AFS::realize_dummy_(int vd_idx) {
...
std::optional<VD_AFS::Candidate> VD_AFS::find_best_candidate_() const {
  ...
  for (int d = 0; d < L_; ++d) {
    ...
    if (ac > best.abs_corr) {
      best = {Candidate::Pool::VD, d, ac, true};
    }
  }

  // Scan realized dummies (always active, can be re-selected)
...
// ========================================================================
// AFS-specific: append column to Cholesky (only on new activations)
// ========================================================================
...
// ========================================================================
// AFS-specific: OLS solve on active set → cache in nu_active_
// ========================================================================
void VD_AFS::ols_solve_() {
  ...
  chol_factor_.topLeftCorner(k, k)
    ...
    .solveInPlace(nu_active_);
  chol_factor_.topLeftCorner(k, k)
    .template triangularView<Eigen::Upper>()
    .solveInPlace(nu_active_);
  // nu_active_ now holds ν̂ = (X_A^T X_A)^{-1} X_A^T y

  nu_stale_ = false;
}

// ========================================================================
// AFS-specific: blend and refresh
// ========================================================================
void VD_AFS::afs_blend_() {
  ...
  // ---- Scatter ν̂ into full vectors ----
  // We need X_A ν̂ for the μ update.  Compute it first.
  Vec Xa_nu(n_);
  Xa_nu.noalias() = X_active_.leftCols(k) * nu_active_;

  // ---- Incremental μ update: μ = (1-ρ) μ + ρ X_A ν̂ ----
  mu_ = (1.0 - rho_) * mu_ + rho_ * Xa_nu;

  // ---- Blend β for real features ----
  beta_ *= (1.0 - rho_);
  for (int i = 0; i < k; ++i) {
...
MatC VD_AFS::run(int T) {
  ...
  for (int it = step_; it < max_steps; ++it) {
    ...
    // 1. Find best candidate (scans all features, including active)
    ...
    if (cand.is_new) {
      ...
      // Extend Cholesky and mark OLS as stale
      append_to_factor_(x_col);
      nu_stale_ = true;
    }
    // If re-selection of an active feature: no structural change,
    // nu_active_ from previous solve is still valid.

    // 3. AFS blend: β = (1-ρ)β + ρ ν̂, update μ, r, correlations
    afs_blend_();
